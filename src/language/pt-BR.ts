...
exports.errors = {
    ...
    array: {
        ...
        includesSingle:
            'o valor único de "{{! label}}" não corresponde a nenhum dos tipos permitidos ',
        ...
        includesRequiredUnknowns:
            'não contém {{unknownMisses}} valor (es) necessário (s)',
        includesRequiredKnowns: 'não contém {{knownMisses}}',
        includesRequiredBoth:
            'não contém {{knownMisses}} e {{unknownMisses}} outros valores obrigatórios',
        excludes: 'na posição {{pos}} contém um valor excluído',
        excludesSingle: 'valor único de "{{! label}}" contém um valor excluído',
        hasKnown:
            'não contém pelo menos uma correspondência necessária para o tipo "{{! patternLabel}}"',
        ...
        ordersLength:
            'na posição {{pos}} falha porque o array deve conter no máximo {{limit}} itens',
    ...
    date: {
        base:
            'deve ser um número de milissegundos ou uma sequência de datas válida',
        ...
        timestamp: {
            javascript:
                'deve ser um carimbo de data / hora válido ou o número de milissegundos',
    ...
    lazy: {
        ...
        schema:
            '!!erro de esquema: a função de esquema lento deve retornar um esquema',
    },
    object: {
        ...
        without:
            '!! "{{mainWithLabel}}" entra em conflito com o par proibido "{{peerWithLabel}}"',
        missing: 'deve conter pelo menos um dos {{peersWithLabels}}',
        xor: 'contém um conflito entre pares exclusivos {{peersWithLabels}}',
        oxor:
            'contém um conflito entre pares exclusivos opcionais {{peersWithLabels}}',
        and:
            'contém {{presentWithLabels}} sem os pares necessários {{missingWithLabels}}',
        nand:
            '!! "{{mainWithLabel}}" não deve existir simultaneamente com {{peersWithLabels}}',
        assert:
            '!! "{{ref}}" a validação falhou porque "{{ref}}" falhou ao {{message}}',
        rename: {
            multiple:
                'não é possível renomear o filho "{{from}}" porque várias renomeações estão desativadas e outra chave já foi renomeada para "{{to}}"',
            override:
                'não é possível renomear o filho "{{from}}" porque a substituição está desativada e o destino "{{to}}" existe ',
            regex: {
                multiple:
                    'não é possível renomear filhos {{from}} porque várias renomeações estão desativadas e outra chave já foi renomeada para "{{to}}"',
                override:
                    'não é possível renomear filhos {{from}} porque a substituição está desativada e o destino "{{to}}" existe',
            },
        },
    ...
    string: {
        ...
        regex: {
            base:
                'com o valor "{{! value}}" falha ao corresponder ao padrão necessário: {{pattern}}',
            name: 'with value "{{! value}}" falha ao corresponder ao padrão {{name}}',
            inverter: {
                base:
                    'with value "{{! value}}" corresponde ao padrão invertido: {{pattern}}',
        ...
        uriCustomScheme:
            'deve ser um uri válido com um esquema que corresponda ao padrão {{schema}}',
        isoDate: 'deve ser uma data ISO 8601 válida',
        guid: 'deve ser um GUID válido',
        hex: 'deve conter apenas caracteres hexadecimais',
        hexAlign:
            'a representação decodificada hexadecimal deve ser alinhada por bytes',
        ...
        ipVersion:
            'deve ser um endereço IP válido de uma das seguintes versões {{version}} com um CIDR {{cidr}}',
    },
    symbol: {
